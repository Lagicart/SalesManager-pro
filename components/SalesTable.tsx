...
interface SalesTableProps {
  ...
  onUpdateNotizie?: (id: string, notizia: string, nuoveNotizie: boolean) => void;
...
const SalesTable: React.FC<SalesTableProps> = ({ vendite, metodiDisponibili, isAdmin, onIncasso, onEdit, onDelete, onCopy, onUpdateNotizie, currentUserNome }) => {
  const [filters, setFilters] = useState({
    cliente: '', agente: '', data: '', metodiSelezionati: [] as string[],
    status: 'all' as 'all' | 'incassato' | 'pendente',
    noteType: 'all' as 'all' | 'ok-marilena' | 'empty'
  });

  // Added resetFilters function to fix the "Cannot find name 'resetFilters'" error
  const resetFilters = () => {
    setFilters({
      cliente: '',
      agente: '',
  ...
  const filteredData = useMemo(() => {
    let list = [...vendite].filter(v => {
      ...
      const matchMetodo = filters.metodiSelezionati.length > 0 ? filters.metodiSelezionati.includes(v.metodoPagamento) : true;
      const matchStatus = filters.status === 'all' ? true : filters.status === 'incassato' ? v.incassato : !v.incassato;
      let matchNote = true;
      if (filters.noteType === 'ok-marilena') matchNote = v.noteAmministrazione?.includes('OK MARILENA');
      else if (filters.noteType === 'empty') matchNote = !v.noteAmministrazione || v.noteAmministrazione.trim() === '';
  ...
  const handleOpenChat = (v: Vendita) => {
    setActiveChat(v);
    // Se ci sono nuove notizie, segniamole come lette appena si apre il popup
    if (v.nuove_notizie && onUpdateNotizie) {
      onUpdateNotizie(v.id, v.notizie || '', false);
    }
  };

  const handleSendMessage = () => {
    if (!newMessage.trim() || !activeChat || !onUpdateNotizie) return;
    
    const timestamp = new Date().toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' });
    const formattedMsg = `[${currentUserNome || 'Utente'}|${timestamp}] ${newMessage.trim()}`;
    const updatedNotizie = activeChat.notizie ? `${activeChat.notizie}\n${formattedMsg}` : formattedMsg;
    
    onUpdateNotizie(activeChat.id, updatedNotizie, true); // true = nuove notizie per gli altri
    setNewMessage('');
    setActiveChat({...activeChat, notizie: updatedNotizie, nuove_notizie: true});
  };
    ...
    <div className="space-y-4">
      ...
      {selectedReceipt && (
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-xl z-[300] flex items-center justify-center p-6 no-print animate-in fade-in duration-300">
          <div className="absolute top-6 right-6 flex gap-4">
            <button onClick={() => setSelectedReceipt(null)} className="bg-white/10 hover:bg-white/20 text-white p-3 rounded-2xl transition-all"><X className="w-6 h-6" /></button>
          </div>
          <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl overflow-hidden border-[12px] border-emerald-50 text-center p-10 space-y-8">
            ...
            {selectedReceipt.noteAmministrazione && (
              <div className="bg-emerald-900 text-white p-6 rounded-2xl transform rotate-1 shadow-lg">
                <p className="text-[10px] font-black uppercase tracking-widest mb-1 opacity-50">Amministrazione</p>
                <p className="text-lg font-black uppercase tracking-widest">{selectedReceipt.noteAmministrazione}</p>
              </div>
            )}
          </div>
        </div>
      ...
      {activeChat && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[300] flex items-center justify-center p-6 no-print animate-in zoom-in-95">
          <div className="bg-[#f0f2f5] w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col h-[80vh]">
            <div className="bg-sky-600 p-6 text-white flex justify-between items-center shadow-md">
              <div className="flex items-center gap-3">
                <div className="bg-white/20 p-2 rounded-xl"><MessageSquare className="w-5 h-5" /></div>
                <div>
                  <h3 className="font-bold text-lg leading-tight">{activeChat.cliente}</h3>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
              {activeChat.notizie ? activeChat.notizie.split('\n').map((line, i) => {
                ...
                if (!parts) return <div key={i} className="text-xs bg-white p-3 rounded-2xl shadow-sm text-slate-600 border border-slate-100">{line}</div>;
                const [_, sender, time, text] = parts;
                const isMe = sender === currentUserNome;
                return (
                  <div key={i} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                    <div className={`max-w-[85%] p-4 rounded-2xl shadow-sm relative ${isMe ? 'bg-emerald-500 text-white rounded-tr-none' : 'bg-white text-slate-800 rounded-tl-none border border-slate-200'}`}>
                      {!isMe && <p className="text-[9px] font-black uppercase tracking-widest text-sky-600 mb-1">{sender}</p>}
                      <p className="text-sm font-medium leading-relaxed">{text}</p>
                      <p className={`text-[9px] mt-2 opacity-50 text-right ${isMe ? 'text-white' : 'text-slate-400'}`}>{time}</p>
                    </div>
                  </div>
                );
              }) : (
                <div className="flex flex-col items-center justify-center h-full text-slate-400 italic space-y-2">
                  ...
                  <p className="text-sm">Nessun messaggio. Scrivi la prima notizia!</p>
                </div>
              )}
              <div ref={chatEndRef} />
            </div>
            ...
            <div className="p-6 bg-white border-t border-slate-200">
              <div className="flex gap-2">
                <input 
                  type="text" placeholder="Scrivi una notizia..."
                  className="flex-1 bg-slate-100 border-none rounded-2xl px-5 py-3.5 text-sm font-medium outline-none focus:ring-2 focus:ring-sky-500/20 transition-all"
                  value={newMessage}
                ...
                <button 
                  ...
                  className="bg-sky-600 text-white p-3.5 rounded-2xl shadow-lg hover:bg-sky-700 active:scale-90 transition-all"
                >
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 filter-panel no-print">
        <div className="flex items-center gap-2 text-slate-800 font-bold"><Filter className="w-4 h-4 text-[#32964D]" /><span>Filtri Ricerca</span></div>
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-3 mt-4">
          ...
          <button onClick={resetFilters} className="px-4 py-2 bg-slate-100 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-200">Reset</button>
        </div>
      </div>
                ...
                <tr key={v.id} className={`hover:bg-slate-50 transition-colors ${v.incassato ? 'bg-emerald-50/20' : ''}`}>
                  ...
                  <td className="px-6 py-4 text-center">
                    <button 
                      ...
                      className={`p-2.5 rounded-2xl transition-all relative ${v.nuove_notizie ? 'bg-sky-600 text-white shadow-lg shadow-sky-600/30' : (v.notizie ? 'bg-slate-100 text-slate-600' : 'text-slate-300 hover:text-slate-500')}`}
                    >
                      <MessageSquare className={`w-5 h-5 ${v.nuove_notizie ? 'animate-pulse' : ''}`} />
                      {v.nuove_notizie && <span className="absolute top-0 right-0 w-3 h-3 bg-rose-500 border-2 border-white rounded-full"></span>}
                    </button>
                  </td>
                  ...
                  <td className="px-6 py-4 no-print text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => setSelectedReceipt(v)} className="p-2.5 text-slate-400 hover:bg-emerald-50 hover:text-[#32964D] rounded-xl transition-all"><Camera className="w-5 h-5" /></button>
