...
const AgentManager: React.FC<AgentManagerProps> = ({ agenti, operatori, isAdmin, currentUser, onUpdate, onDelete }) => {
  ...
  const filteredAgentList = useMemo(() => {
    ...
    if (operatorFilter !== 'all') list = list.filter(a => a.operatoreEmail.toLowerCase() === operatorFilter.toLowerCase());
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Controllo conflitto assegnazione
    if (existingAgent && 
      ...
      <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-200">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8">
          <div>
            ...
            <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mt-1">Anagrafica centralizzata</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-5 bg-slate-50 rounded-3xl border border-slate-100">
          <div className="relative md:col-span-1"><Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" /><input type="text" placeholder="Cerca..." className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-2xl text-xs font-bold outline-none focus:border-[#32964D]" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} /></div>
          ...
          <select className="w-full px-4 py-3 bg-white border border-slate-200 rounded-2xl text-xs font-black uppercase outline-none focus:border-[#32964D]" value={operatorFilter} onChange={e => setOperatorFilter(e.target.value)}><option value="all">Tutti gli Operatori</option>{operatori.map(op => <option key={op.email} value={op.email}>{op.nome}</option>)}</select>
          <button onClick={() => { setSearchTerm(''); setZoneFilter('all'); setOperatorFilter('all'); }} className="flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-[#32964D]"><RotateCcw className="w-4 h-4" /> Reset</button>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Operatore</th>
        ...
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-xl z-[400] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] w-full max-w-md shadow-2xl overflow-hidden border border-white/20">
            ...
            <form onSubmit={handleSubmit} className="p-10 space-y-6">
              ...
              <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Assegna a Operatore</label><select required className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 text-xs font-black uppercase outline-none focus:border-[#32964D]" value={formData.operatoreEmail} onChange={e => setFormData({...formData, operatoreEmail: e.target.value})}>{operatori.map(op => <option key={op.id} value={op.email}>{op.nome}</option>)}</select></div>
